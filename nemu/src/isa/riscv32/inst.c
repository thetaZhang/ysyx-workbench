...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((signed)src1 >= (signed)src2) ? (s->pc + imm) : s->dnpc);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, sizeof(word_t)/(sizeof(uint8_t) * 2), src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, sizeof(word_t)/sizeof(uint8_t), src2));
