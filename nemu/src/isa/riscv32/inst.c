...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = (sword_t)((signed)src1 * (signed)src2));
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = BITS(((int64_t)(signed)src1 * (int64_t)(signed)src2), 63, 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = BITS(((int64_t)(signed)src1 * (uint64_t)src2), 63, 32));  
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (signed)src1 >> BITS(imm, 4, 0));
  
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, sizeof(word_t)/(sizeof(uint8_t) * 2)), 8 * sizeof(word_t)/(sizeof(uint8_t) * 2)));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, sizeof(word_t)/sizeof(uint8_t)), 8 * sizeof(word_t)/sizeof(uint8_t)));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, sizeof(word_t)/(sizeof(uint8_t) * 2)));
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = ECALL(s->dnpc, R(17))); // R(17) is $a7 // software doesn't process the epc, so we should store dnpc to epc so that can return correctly
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , I, s->dnpc = SR(0x341)); // mepc
